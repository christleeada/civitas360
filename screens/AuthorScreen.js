...
const AuthorScreen = () => {
  ...
  const fetchData = async author => {
    try {
      const response = await axios.get(`${API_URL}/items-author/0/${author}`);
  ...
  useEffect(() => {
    ...
    setAuthor(route.params ? route.params.author.id : '');

    NetInfo.addEventListener(state => {
      setIsConnected(state.isConnected);
    });

    fetchData(route.params ? route.params.author.id : '');
